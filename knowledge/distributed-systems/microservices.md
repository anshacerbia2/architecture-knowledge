...
forces:
  - Independent deployment needs stable contracts.
  - Network calls fail and add latency.
  - Data ownership complicates cross-service consistency.
  - Team topology shapes boundaries.
...
quality_attributes:
  ...
  influences: []
constraints:
  - statement: Independent deployment must be supported by service ownership, delivery, data, and operating capability.
    scope: edge-local
    concept_ids: []
assumptions:
  - statement: Teams can operate networked services and independently owned state across the full lifecycle.
    scope: edge-local
    concept_ids: []
benefits:
  - Independent deployment and scaling.
  - Clear service ownership.
  - Fault and technology isolation when boundaries hold.
tradeoffs:
  - Network and platform complexity.
  - Harder cross-service consistency.
  - More demanding observability and testing.
risks:
  - statement: A distributed monolith combines coupled releases with network failure.
    scope: edge-local
    concept_ids: []
failure_modes:
  - AKC-000021
...
alternatives:
  - statement: A modular monolith retains internal modularity and one primary deployment boundary.
...
relationships:
  ...
  - AKR-000018
examples:
  - statement: Capability-aligned teams own independently deployed services and their data, with contracts and operational responsibility.
    scope: edge-local
    concept_ids: []
counterexamples:
  - statement: Many processes that require coordinated releases and share one mutable schema form a distributed monolith, not effective autonomy.
    scope: edge-local
    concept_ids: []
claims:
  - AKL-000008
...
Modular Monolith is an alternative decomposition/deployment choice, Event-Driven Architecture is a compatible interaction style, and constraints determine feasible service boundaries.
